...
import { jiaowuTools } from '../src/core/tool/jiaowu-tools';
...
async function main() {
  ...
  // 监听 Agent 事件
  agent.on('tool:call', ({ toolName, input }) => {
    const inputStr = JSON.stringify(input);
    const display = inputStr.length > 120 ? inputStr.slice(0, 120) + '...' : inputStr;
    process.stdout.write(`\n[🔧 工具调用] ${toolName}(${display})\n`);
  });
  agent.on('tool:result', ({ toolName, duration }) => {
    process.stdout.write(`[✅ 工具完成] ${toolName} (${duration}ms)\n`);
  });
  agent.on('tool:error', ({ toolName, error }) => {
    process.stdout.write(`[❌ 工具错误] ${toolName}: ${error.message}\n`);
  });
  ...
  gateway.onMessage(async (envelope) => {
    try {
      const output = await agent.complete({ prompt: envelope.text });
      await gateway.send({
        conversationId: 'debug-console:main',
