...
import {
  extractTeacherCourseInfo,
  extractTeacherSearchResults,
 ...
 * 教师姓名搜索 URL
 */
const TEACHER_SEARCH_URL = "https://jiaowu.sicau.edu.cn/web/web/lanmu/jiaoshi_new.asp";

/**
 * 教师课表详情 URL
 */
const TEACHER_COURSE_URL = "https://jiaowu.sicau.edu.cn/web/web/lanmu/jiaoshikb_new.asp";
...
export const getTeacherCourseInfoByName = async (
  ...
  try {
    ...
    const searchHtml = await fetchJiaowuHtml(`${TEACHER_SEARCH_URL}?xm=${encodedName}&shou1=`);
    const searchResult = extractTeacherSearchResults(searchHtml);

    // 2. 按姓名精确匹配
    const match = searchResult.result?.find((item) => item.name === name);
    if (!match) {
