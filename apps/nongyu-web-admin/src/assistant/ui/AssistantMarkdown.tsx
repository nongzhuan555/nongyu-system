...
type AssistantMarkdownProps = {
  content: string;
  /** 流式中对展示字符串做 remend；落盘仍用原始 content */
  isStreaming?: boolean;
};

/**
 * 助手正文 Markdown。
 * 流式时 remend 后再交给 react-markdown，避免半截 ** / ` / 链接错排。
 */
export function AssistantMarkdown({ content, isStreaming = false }: AssistantMarkdownProps) {
  if (!content.trim()) return null;

  const source = isStreaming ? remend(content, STREAM_HEAL_OPTIONS) : content;
