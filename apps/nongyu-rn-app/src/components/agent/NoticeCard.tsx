...
import { getNoticeInfo } from "nongyu-tool-jiaowu";
import type { ToolRenderProps } from "@/agent-ui/registry";

type NoticeResult = Awaited<ReturnType<typeof getNoticeInfo>>;

function NoticeCardInner({ output, status, error }: ToolRenderProps<{}, NoticeResult>) {
  ...
  if (status === "error") {
    return (
      <View style={[styles.card, styles.errorCard]}>
        <Text style={styles.errorText}>⚠ {error ?? "查询教务通知失败"}</Text>
      </View>
    );
  }
  ...
  if (!output.success || !Array.isArray(output.result) || output.result.length === 0) {
    return (
      <View style={[styles.card, styles.emptyCard]}>
        <Text style={styles.emptyText}>暂无教务通知</Text>
      </View>
    );
  }
  ...
  const navigate = () => router.push("/home/jiaowu/notice" as Href);
