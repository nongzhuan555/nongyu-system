...
**技术组件**（已抽到 `docs/nongyu-go-track-server/技术选型.md`，此处摘要）：

| 组件   | 结论                                                                           |
| ------ | ------------------------------------------------------------------------------ |
| HTTP   | `net/http` + **`go-chi/chi`**（已锁定，不再在 echo 间摇摆）                    |
| SQLite | `modernc.org/sqlite` + `database/sql`                                          |
| 配置   | 环境变量：`HTTP_ADDR` `DB_PATH` `JWT_SECRET` `INTERNAL_TOKEN` 等（见选型文档） |
| 日志   | slog JSON                                                                      |
...
日常发布路径见 **`docs/nongyu-go-track-server/部署与发布.md`**（本机交叉编译 → 上传二进制 → systemd 重启）。连接口令等只放本机 `服务器连接信息.md` / `scripts/ops/`（不入库）。
...
| 文档                                        | 关系                                                                                       |
| ------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `docs/nongyu-go-track-server/技术选型.md`   | 实现技术底座（chi / modernc / slog 等）；自本方案抽出并锁定                                |
| `docs/nongyu-go-track-server/接口文档.md`   | HTTP 契约真相；自本方案 §9 展开                                                            |
...
| 2026-08-14 | 抽出独立 `技术选型.md` / `接口文档.md`；HTTP 锁定 chi            |
