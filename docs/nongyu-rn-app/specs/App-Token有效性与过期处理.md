...
| 非目标                        | 说明                      |
| ----------------------------- | ------------------------- |
| Refresh / 静默续期            | 不做（见 §4.4 无票补签）  |
| 修改教务登录主流程顺序        | 不改教务→档案→Node 顺序   |
| 本地解析 JWT `exp` 替代服务端 | 以 `/me` 与接口错误码为准 |
| Admin / WebView               | 不在本 Spec               |

### 3.1 补充：登录签发失败与无票补签（2026-08-16）

仍不强制「无 Node 票则禁止进 App」（与 `登录门禁` 一致）。但须保证：

1. **登录请求体清洗**：提交 Node `/api/app/auth/login` 前，将 `hometown`/`campus`/`college`/`major`/`className`/`grade` 等按契约长度截断；`gender` 仅传可识别值，否则省略，避免教务长地址/异常性别导致签发失败而本地仍「已登录」。
2. **无票补签**：已有教务会话档案、但 `session.token`（及 DEV 回落）皆空时，业务层可调用 `ensureAppAccessToken`：用当前档案再打一次登录签发；成功则写入 session + MMKV 快照。
3. **错误文案**：`appFetch` 无票时不得再写「缺少联调 Token」误导生产用户；改为「农屿服务未接通或登录凭证缺失」类表述。
4. **EAS**：`eas.json` production/preview 须注入 `EXPO_PUBLIC_API_BASE_URL`（公网 HTTPS，**禁止** `http://IP:3000`，公网未开 3000）。
...
| 2026-08-16 | 登录字段清洗、无票补签、错误文案与 EAS API Base  |
