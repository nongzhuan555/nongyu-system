...
| 广场 | `post_replies`         | 帖子回复/留言（管理员回复 + 大院留言，kind 区分）         |
| 运营 | `app_versions`         | App 版本发布                                              |
| LLM  | `llm_api_keys`         | 平台 LLM 代理 Key 池（密文）                              |
| LLM  | `llm_user_usage_daily` | 用户平台代理日调用计数                                    |
...
| `active_days`         | INT UNSIGNED | NOT NULL DEFAULT 0 | 累计活跃天数（统计上线后按业务日向前累计，不回填）     |
| `active_day_bucket`   | DATE         | NULL               | 最近一次已计入 `active_days` 的业务日                  |
...
**索引**：`uk_student_no`；`idx_role_status`；`idx_created_at`；`idx_is_online`；`idx_college`；`idx_grade`；`idx_campus`；`idx_gender`；`idx_device_brand`；`idx_last_active_at`；`idx_active_days`。

**业务规则**：

- 不存教务密码。
- 新设备登录成功：更新 `current_device_id` / 设备字段，并 `token_version = token_version + 1`，使旧 JWT 失效；`is_online = 1`，刷新 `last_active_at` / `last_login_at`；若业务日相对 `active_day_bucket` 变化则 `active_days+=1`。
- 主动登出：`is_online = 0`，`token_version` 递增。
- 心跳（经 Track 上报后回写 Node，或 Node 直收）：`is_online = 1`，刷新 `last_active_at`；同业务日去重累计 `active_days`。
- 超时离线：超过约定无心跳窗口（**10 分钟**）将 `is_online = 0`。路径：① Track 超时扫描回调本内部接口；② 管理端 `GET /api/admin/dashboard/overview` 读取前按 `last_active_at` 清理过期在线（防 Track→Node 回写失败导致「无人却显示在线」）。
...
### 4.4 `post_replies` — 帖子回复/留言

`kind` 区分两类回复：

| 值            | 含义             | 适用 post_type | 数量约束 | 作者可见性（App）          | 作者可见性（Admin） |
| ------------- | ---------------- | -------------- | -------- | -------------------------- | ------------------- |
| `admin_reply` | 反馈墙管理员回复 | `feedback`     | 1:1      | 仅「管理员回复」           | 实名                |
| `comment`     | 大院用户留言     | `courtyard`    | 1:N      | 完全匿名（仅 isMine 自识） | 实名                |

| 字段                        | 类型        | 约束                             | 说明                                                   |
| --------------------------- | ----------- | -------------------------------- | ------------------------------------------------------ |
| `id`                        | BIGINT      | PK AI                            |                                                        |
| `post_id`                   | BIGINT      | FK → posts.id ON DELETE CASCADE  |                                                        |
| `kind`                      | VARCHAR(16) | NOT NULL                         | `admin_reply` / `comment`                              |
| `author_user_id`            | BIGINT      | FK → users.id ON DELETE RESTRICT | 管理员回复时为管理员 userId；用户留言时为留言者 userId |
| `content`                   | MEDIUMTEXT  | NOT NULL                         | admin_reply ≤ 2000；comment ≤ 1000                     |
| `notified_author`           | TINYINT(1)  | NOT NULL DEFAULT 0               | 是否已通知帖子作者（0=待通知，轮询后置 1）             |
| `deleted_at`                | DATETIME(3) | NULL                             | 软删                                                   |
| `created_at` / `updated_at` | DATETIME(3) | NOT NULL                         |                                                        |

**索引**：

- `idx_replies_post_kind_created (post_id, kind, created_at)` — 列表/详情按 kind 过滤、时间排序
- `idx_replies_author_created (author_user_id, created_at)` — 用户留言自查
- `idx_replies_notified (notified_author, post_id)` — 轮询新回复

**约束**：

- `chk_replies_kind CHECK (kind IN ('admin_reply','comment'))`
- `admin_reply` 一对一：应用层在事务内 `SELECT ... FOR UPDATE` + 唯一存在性校验保证（无 DB 唯一索引，因软删历史保留）
- 软删帖子时应用层事务级联软删其所有 `post_replies`（`deleted_at` 同步置位）
- `PATCH admin_reply` 不重置 `notified_author`（已通知过的作者不因编辑再次被提醒）
...
| 帖子详情       | `posts` + `post_replies` by `post_id` + `kind` + `created_at`         |
| 我的帖子回复数 | `post_replies` GROUP BY `post_id` WHERE `deleted_at IS NULL`          |
| 轮询我的新回复 | `post_replies` JOIN `posts` ON author = me, `notified_author = 0`     |
...
CREATE TABLE users (
  ...
  active_days INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '累计活跃天数',
  active_day_bucket DATE NULL COMMENT '最近计入的业务日',
  ...
  KEY idx_users_active_days (active_days),
...
- 大屏「当前在线」统计 `users.is_online = 1` **且** `last_active_at` 在近 10 分钟内；须保证登录/登出/心跳/超时路径维护正确。
...
| 2026-08-17 | 新增 `post_replies` 表：支持反馈墙管理员回复（1:1）与大院用户留言（1:N），含 `notified_author` 轮询位 |
