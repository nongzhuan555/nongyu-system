...
**Emails**: Derive from names 鈥?`sarah.chen@example.com`. Use `example.com` domain (RFC 2606 reserved).
...
**Descriptions/content**: Write 3-5 realistic variations per content type and cycle through them. Don't generate AI-sounding prose 鈥?write like real user data.
...
| 100 bound parameter limit    | Keep batch size 脳 columns < 100 |

## Quality Rules

1. **Match the domain** 鈥?an e-commerce seed has products with real-sounding names and prices, not "Product 1"
2. **Vary the data** 鈥?don't make every user "John Smith" or every price "$9.99"
3. **Include edge cases** (for testing seeds) 鈥?empty strings, very long text, special characters, maximum values
4. **Reference real IDs** 鈥?foreign keys must point to actually-inserted parent rows
5. **Print what was seeded** 鈥?always log counts so the user knows it worked
6. **Document the run command** 鈥?put it in a comment at the top of the file
